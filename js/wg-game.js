...
(function(){'use strict';
  ...
  function openSettingsModal() {
    ...
    const html =
      ...
      '<div style="font-size:11px;color:#a89878;line-height:1.7;padding:2px 0;">' +
        '<div>Unlimited Chaos v0.35.1 · Build 2026-05-08</div>' +
  ...
  function initBossIntro() {
    ...
    WG.Engine.on('enemy:killed', ({ creature }) => {
      if (!huntRuntime) return;
      const sd = creature._typeData && creature._typeData.splitOnDeath;
      if (sd) {
        for (let i = 0; i < sd.count; i++) {
          const ang = Math.PI * 2 * (i / sd.count) + Math.random() * 0.5;
          const e = WG.HuntEnemies.spawn(sd.type, creature.x + Math.cos(ang) * sd.spread, creature.y + Math.sin(ang) * sd.spread);
          if (e) {
            const scale = WG.HuntEnemies.TUNABLES.SPLIT_FRAGMENT_HP_SCALE;
            e.hp = Math.round(e.hp * scale);
            e.maxHp = e.hp;
            huntRuntime.creatures.push(e);
          }
        }
      }
